...
function publishActivity(slug) {
    Swal.fire({
        title: 'Publier cette activitée ?',
        icon: 'question',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            router.put(route('owner.activity.published', { slug }), {}, {
                preserveScroll: true,
                onSuccess: () => {
                    // mise à jour directe du tableau
                    const index = props.data.activity.data.findIndex(f => f.slug === slug)
                    if (index !== -1) {
                        props.data.activity.data[index].status = 'published'
                    }
                    Swal.fire({
                        title: 'Publiée !',
                        icon: 'success',
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm sm:text-base text-gray-800">
                            <tr v-for="(item, index) in data.activity.data" :key="item.id"
                                ...
                                <td class="td space-x-3">
                                    ...
                                    <button @click="publishActivity(item.slug)"
                                    ...
                                    <button @click="deleteActivity(item.slug)"
