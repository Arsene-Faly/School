...
function ArchivedFormation(slug) {
    Swal.fire({
        title: 'Arcchiver cette formation ?',
        icon: 'question',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            router.put(route('owner.formation.archived', { slug }), {}, {
                preserveScroll: true,
                onSuccess: () => {
                    // mise à jour directe du tableau
                    const index = props.data.formations.data.findIndex(f => f.slug === slug)
                    if (index !== -1) {
                        props.data.formations.data[index].status = 'archived'
                    }
                    Swal.fire({
                        title: 'Archivée !',
                        icon: 'success',
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm sm:text-base text-gray-800">
                            <tr v-for="(item, index) in data.formations.data" :key="item.id"
                                ...
                                <td class="td space-x-3">
                                    ...
                                    <button @click="ArchivedFormation(item.slug)"
                                    ...
                                    <button @click="deleteFormation(item.slug)"
