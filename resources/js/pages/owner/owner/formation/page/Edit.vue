...
const form = useForm({
    school_id: props.formation.school_id,
    title: props.formation.title || '',
    description: props.formation.description || '',
    cover_photo: null,
    level: props.formation.levels ?? [],
    duration: props.formation.duration || '',
    mode: props.formation.mode || '',
    language: props.formation.language || 'fr',
    price: props.formation.price || '',
    category: props.formation.categories ?? [],
    status: props.formation.status || 'draft',
});

const previewCover = ref(props.formation.cover_photo || null);

// Changement de la photo de couverture
function onCoverPhotoChange(event) {
    ...
    previewCover.value = file ? URL.createObjectURL(file) : props.formation.cover_photo || null;
}

// Sélectionner ou désélectionner tous les niveaux
const toggleAllLevels = () => {
    if (!props.niveaux) return;
    form.level = form.level.length === props.niveaux.length ? [] : [...props.niveaux];
}
...
function submit() {
    form._method = 'PUT';
    form.post(route('owner.formation.update', { slug: props.formation.slug }), {
        onSuccess: () => {
            Swal.fire({
                icon: 'success',
                title: 'Succès',
    ...
    <div class="pt-30 sm:pt-32 pb-12 px-0">
        <div class="bg-gray-50 shadow-lg w-full mx-auto px-4 sm:px-8 md:px-12 lg:px-16 py-12 sm:py-16">
            ...
            <div class="text-center mb-10">
                <h2 class="text-2xl sm:text-3xl md:text-4xl font-bold text-indigo-800">Modifier la formation</h2>
                <p class="mt-2 text-base sm:text-lg text-gray-600">Modifiez les informations de la formation.</p>
            </div>
            ...
            <form @submit.prevent="submit" class="space-y-6 sm:space-y-8">
                ...
                <!-- Photo de couverture -->
                <div>
                    <Label for="cover_photo" class="text-indigo-600 text-lg font-semibold">Photo de couverture</Label>
                ...
                <!-- Titre -->
                <div>
                    <Label for="title" class="text-sm sm:text-base label">Nom de la formation <span class="text-red-500">*</span></Label>
                ...
                <div>
                    ...
                    <div class="mt-2 mb-2">
                        <label class="inline-flex items-center">
                            <input type="checkbox" :checked="form.level.length === props.niveaux.length" @change="toggleAllLevels" class="mr-2" />
                            Sélectionner tous
                        </label>
                    </div>
                ...
                <div>
                    ...
                    <Input id="duration" v-model="form.duration" type="number" min="1" placeholder="ex: 12" class="input mt-2" />
                ...
                <div>
                    ...
                    <Input id="price" v-model="form.price" type="number" step="0.01" placeholder="ex: 1200.00" class="input mt-2" />
