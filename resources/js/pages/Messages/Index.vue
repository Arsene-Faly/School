...
// Mark as read
// ---------------------------
function markAsRead(senderId: number) {
  fetch(route("messages.markAsRead"), {
    method: "POST",
    headers: {
    ...
    .then(() => {
      const u = usersWhoMessaged.value.find(u => u.id === senderId);
      if (u) u.unread = 0;
    });
}
...
function selectReceiver(userId: number) {
  router.get(route("messages.index", { receiver: userId }), {}, {
    preserveScroll: true,
    preserveState: true,
    onSuccess: () => {
      markAsRead(userId);
...
function sendMessage() {
  if (!form.content || !form.receiver_id || isSending.value) return;
...
<UserLayout>
  <div class="flex min-h-screen bg-gray-100 pt-24">
    <!-- Sidebar -->
    <aside class="w-full md:w-1/4 border-r p-4 bg-white shadow-sm rounded-xl overflow-y-auto sticky top-24 h-[calc(100vh-6rem)]">
      ...
      <ul>
        <li v-for="user in usersWhoMessaged" :key="user.id" class="mb-2">
          <button @click="selectReceiver(user.id)"
            ...
            <span v-if="user.unread && user.unread > 0" class="ml-auto text-xs bg-red-500 text-white px-2 py-0.5 rounded-full">
              {{ user.unread }}
            </span>
          </button>
        </li>
    ...
    <main class="flex-1 flex flex-col bg-white shadow-inner rounded-tr-xl rounded-br-xl">
      <header class="bg-blue-600 px-6 py-4 shadow-md sticky top-0 z-10 flex items-center justify-between rounded-tl-xl rounded-tr-xl">
      ...
      <section ref="messagesContainer" class="flex-1 overflow-y-auto px-6 py-4 flex flex-col space-y-4 scrollbar-thin scrollbar-thumb-blue-400 scrollbar-track-blue-100">
        <div v-if="allMessages.length > displayedMessages.length" class="flex justify-center mb-2">
          <button @click="loadPreviousMessages" class="bg-blue-100 text-blue-700 px-4 py-2 rounded-full hover:bg-blue-200 transition">
        ...
        <div v-for="msg in displayedMessages" :key="msg.id" class="flex items-end"
          ...
          <div v-if="msg.sender_id !== page.props.auth.user.id" class="mr-2">
            <div class="w-8 h-8 rounded-full bg-blue-400 flex items-center justify-center text-white font-semibold text-sm">
          ...
          <div :class="msg.sender_id === page.props.auth.user.id
            ...
            <div v-if="msg.sender_name && msg.sender_id !== page.props.auth.user.id" class="text-sm font-semibold text-blue-600 mb-1">
              {{ msg.sender_name }}
            </div>
            <div>{{ msg.content }}</div>
      ...
      <footer v-if="receiver?.id" class="mt-4 flex gap-3 px-6 py-4 bg-white sticky bottom-0 rounded-bl-xl rounded-br-xl shadow-inner">
