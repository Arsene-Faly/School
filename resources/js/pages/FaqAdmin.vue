...
<template>
    <SidebarLayout :user-role="readableRole" activePage="FAQ" welcome="Bienvenue dans le tableau de Propriétaire d'école"
                    ...
                    <div v-for="item in section.items" :key="item.id" class="border-b border-gray-300">
                        <button @click="toggle(item.id)"
                            ...
                            <svg :class="{ 'rotate-180': openItem === item.id }"
                                class="w-5 h-5 text-indigo-600 transform transition-transform duration-200" fill="none"
